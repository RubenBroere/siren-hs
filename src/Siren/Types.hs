...
data Node = Node
    ...
    , nodeLabel :: String     -- ^ Display label for the node
    , nodeShape :: NodeShape  -- ^ Visual shape for rendering
    }
    deriving (Eq, Show)

-- | Represents a directed edge connecting two nodes.
data Edge = Edge
    { edgeFrom :: NodeId       -- ^ Source node identifier
    , edgeTo :: NodeId         -- ^ Target node identifier
    , edgeLabel :: Maybe String -- ^ Optional label to display on the edge
    }
    deriving (Eq, Show)
...
data Graph = Graph
    { graphNodes :: Map NodeId Node  -- ^ All nodes in the graph, indexed by their IDs
    , graphEdges :: [Edge]           -- ^ All edges in the graph
    }
    deriving (Eq, Show)
...
insertNode node graph =
    graph {graphNodes = Map.insert (nodeId node) node (graphNodes graph)}
...
insertEdge edge graph =
    graph {graphEdges = edge : graphEdges graph}
