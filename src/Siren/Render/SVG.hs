...
renderGraphToFile :: FilePath        -- ^ Output file path (should end in .svg)
                  -> LaidOutGraph    -- ^ The graph with positioned nodes
                  -> IO ()
renderGraphToFile outputPath graph =
    let (minX, maxX, minY, maxY) = layoutBounds graph
        canvasPadding = 1.4
        ...
        canvasWidthPx = canvasWidthUnits * unitsPerLayoutUnit
        canvasHeightPx = canvasHeightUnits * unitsPerLayoutUnit
     in renderSVG outputPath (dims2D canvasWidthPx canvasHeightPx) (drawGraph graph)
...
drawNode geometry =
    let currentNode = geometryNode geometry
        ...
        styledShape = baseShape # fc white # lc black # lwG nodeLineWidth
        labelDiagram = text (nodeLabel currentNode) # fontSizeL nodeTextSize # fc black # centerXY
             in (labelDiagram <> styledShape) # translate (r2 (centerCoordX, centerCoordY))
...
drawEdge nodeGeometries currentEdge =
    case (Map.lookup (edgeFrom currentEdge) nodeGeometries, Map.lookup (edgeTo currentEdge) nodeGeometries) of
        (Just sourceGeometry, Just targetGeometry) ->
            let (sourcePoint, targetPoint) = clippedEdgeEndpoints sourceGeometry targetGeometry
                arrowStyle = with & headLength .~ global arrowHeadLength & shaftStyle %~ lwG arrowShaftWidth
             in arrowBetween' arrowStyle sourcePoint targetPoint
        _ -> mempty

drawEdgeLabel :: Map NodeId NodeGeometry -> Edge -> QDiagram B V2 Double Any
drawEdgeLabel nodeGeometries currentEdge =
    case (Map.lookup (edgeFrom currentEdge) nodeGeometries, Map.lookup (edgeTo currentEdge) nodeGeometries, edgeLabel currentEdge) of
        (Just sourceGeometry, Just targetGeometry, Just labelText) ->
            let (sourcePoint, targetPoint) = clippedEdgeEndpoints sourceGeometry targetGeometry
                edgeVector = targetPoint .-. sourcePoint
                perpendicular = normalizedPerp edgeVector
                alongEdge = safeNormalized edgeVector
                -- Choose perpendicular direction based on which side of the edge is more empty
                -- For better visibility, prefer the direction that has larger absolute x or y component
                V2 px py = perpendicular
                perpendicularDir = if abs px > abs py then perpendicular else negated perpendicular
                ...
                labelBackground =
                   ...
                   in (labelGlyph <> labelBackground # opacity 0.96) # moveTo labelPoint
        _ -> mempty
...
clippedEdgeEndpoints sourceGeometry targetGeometry =
    let sourceCenter = geometryCenter sourceGeometry
...
boundaryDistance geometry unitDirection =
  ...
  where
    ...
    boxDistance =
        let scaleToVerticalBoundary = if abs dx < epsilon then 1 / epsilon else halfWidth / abs dx
            scaleToHorizontalBoundary = if abs dy < epsilon then 1 / epsilon else halfHeight / abs dy
         in min scaleToVerticalBoundary scaleToHorizontalBoundary
