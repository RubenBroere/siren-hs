...
{-|
Module      : Siren.Layout.Sugiyama
Description : Sugiyama-style layered layout algorithm
Copyright   : (c) Ruben Broere
License     : See LICENSE file
Maintainer  : ruben.broere@example.com
Stability   : experimental

This module implements a simplified Sugiyama-style hierarchical layout algorithm
...
-- | Configurable Sugiyama-like layout engine.
newtype SugiyamaLikeLayout = SugiyamaLikeLayout
  { sugiyamaDirection :: Direction
  }
  deriving (Eq, Show)

instance LayoutEngine SugiyamaLikeLayout where
  runLayout (SugiyamaLikeLayout direction) = layoutSugiyamaLike direction
...
layoutSugiyamaLike direction graph =
  withComputedMetrics (graphEdges graph) positionedNodes
  where
    ...
    uniformWidth = layoutUniformWidthFromNodes rankedNodes
    positionedNodes = fmap (mkPositionedNode rankMap positionMap uniformWidth) rankedNodes
...
mkPositionedNode :: Map NodeId Int -> Map NodeId (Double, Double) -> Double -> Node -> PositionedNode
mkPositionedNode rankMap positionMap uniformWidth currentNode =
  PositionedNode
    { positionedNode = currentNode
    , positionedX = xPos
    , positionedY = yPos
    , positionedRank = nodeRank
    , positionedWidth = nodeBoxWidth
    , positionedHeight = nodeBoxHeight
    }
  where
    ...
    (nodeBoxWidth, nodeBoxHeight) = layoutNodeDimensions uniformWidth currentNode
